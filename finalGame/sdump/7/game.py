    ...
    def __init__(self, width=800, height=600, caption="Would you like to play a game?", level=None, resizable=True):
        ...
        def on_mouse_press(x, y, button, modifiers):
            if button & pyglet.window.mouse.LEFT:
                self.mouseTracking['LEFT'] = [x,y,modifiers]
            if button & pyglet.window.mouse.RIGHT:
                self.mouseTracking['RIGHT'] = [x,y,modifiers]
            if button & pyglet.window.mouse.MIDDLE:
                self.mouseTracking['MIDDLE'] = [x,y,modifiers]

        # Handle Mouse Release
        @self.window.event
        def on_mouse_release(x, y, button, modifiers):
            if button & pyglet.window.mouse.LEFT and 'LEFT' in self.mouseTracking:
                del(self.mouseTracking['LEFT'])
            if button & pyglet.window.mouse.RIGHT and 'RIGHT' in self.mouseTracking:
                del(self.mouseTracking['RIGHT'])
            if button & pyglet.window.mouse.MIDDLE and 'MIDDLE' in self.mouseTracking:
                del(self.mouseTracking['MIDDLE'])
        ...
        def on_draw():
            ...
            if self.level is not None:
                heroDeath = self.level.draw(self.worldTime, self.dt, self.width, self.height, self.keyTracking, self.mouseTracking)
                if heroDeath:
